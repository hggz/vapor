// WebSocket routes use WebSocketKit's `WebSocket` and `WebSocketUpgrader` on every platform
// Vapor supports, including Windows MSVC since the NIOSSL Swift port (Phase E).
import RoutingKit
import WebSocketKit
import NIOCore
import NIOHTTP1
